<template>
    ...
    <v-container>
        <!-- ПРИ ЗАГРУЗКЕ СПИСКА -->
            <div class="" v-if="$store.state.restaurants_modules.problem_state_restaurant == 'requested'">
                <div class="loading">
                <h1>Loading...</h1>
        ...
        <!-- ПРИ ОШИБКЕ ЗАГРУЗКИ -->
        <div v-if="$store.state.restaurants_modules.problem_state_restaurant == 'failed'" class="loading-error">
        ...
        <div v-if="$store.state.restaurants_modules.problem_state_restaurant == 'succeeded'">
    <v-container >
...
export default {
  ...
  methods:{
    reload_restourants(){
      store.state.restaurants_modules.problem_state = typesRestaurants.request_status.REQUESTED;
      store.dispatch(typesRestaurants.actions.PARSE_restourants);
      this.dialog = false;
    },
    cancel_parse(){
      store.dispatch(typesRestaurants.actions.CANCEL_PARSE)
    }
  },
  computed:{
